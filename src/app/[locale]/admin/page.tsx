...
export default function AdminPage() {
  ...
  const action = useMutation({
    mutationFn: async ({ id, act }: { id: string; act: "approve" | "reject" }) => {
      const res = await fetch(`/api/admin/requests/${id}/action?token=${encodeURIComponent(activeToken)}`, {
        method: "POST",
    ...
    return (
      <div className="min-h-screen bg-stone-950 flex items-center justify-center px-6">
        <div className="w-full max-w-xs flex flex-col gap-4">
          <h1 className="text-white font-black text-2xl text-center">🔒 Admin Portal</h1>
          <input
            type="password"
            value={token}
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && token) { setActiveToken(token); setAuthed(true); }
            }}
          />
          <button
            onClick={() => { if (token) { setActiveToken(token); setAuthed(true); } }}
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-white">
      ...
      <div className="bg-stone-900 border-b border-stone-800 px-4 py-3 flex items-center justify-between">
        <h1 className="font-black text-lg">🎭 AI Image Admin</h1>
        <button onClick={() => refetch()} className="text-stone-400 hover:text-white text-sm">↻ Refresh</button>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {requests.map((r) => (
            <button
              key={r.id}
              onClick={() => { setPreview(r); setNote(r.admin_note ?? ""); }}
              className="relative flex flex-col rounded-2xl overflow-hidden border border-stone-700 hover:border-orange-500 transition-colors bg-stone-900"
            >
              {/* Selfie thumbnail */}
              ...
              <div className="p-2 flex flex-col gap-1">
                <span className={cn("text-xs font-bold px-2 py-0.5 rounded-full self-start", STATUS_COLORS[r.status] ?? "bg-stone-700 text-stone-200")}>
                  {r.status}
                </span>
      ...
      {/* Preview modal */}
      {preview && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-end sm:items-center justify-center p-4" onClick={() => setPreview(null)}>
          <div
          ...
          >
            ...
            <div className="p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <span className={cn("text-xs font-bold px-2 py-1 rounded-full", STATUS_COLORS[preview.status])}>
                  {preview.status}
                </span>
